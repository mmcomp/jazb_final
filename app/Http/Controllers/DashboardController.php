...
class DashboardController extends Controller
{
    ...
    public function index(){
        ...
        $todayStudents = Student::where('is_deleted', false)->where('supporters_id', 0)->where('created_at', '>=', date('Y-m-d 00:00:00'))->where('created_at', '<=', date('Y-m-d 23:59:59'))->count();
        $year = (int)jdate()->format("Y");
        $month = (int)jdate()->format("m");
        $startOfYear = implode('-', CalendarUtils::toGregorian($year, 1, 1));
        ...
        $results = [];
        for($i = 0;$i < count($dates)-1;$i++){
            $results[] = Student::where('is_deleted', false)->where('created_at', '>=', DashboardController::fixDateDigits($dates[$i]) . ' 00:00:00')->where('created_at', '<', DashboardController::fixDateDigits($dates[$i+1]) . ' 00:00:00')->count();
        }
